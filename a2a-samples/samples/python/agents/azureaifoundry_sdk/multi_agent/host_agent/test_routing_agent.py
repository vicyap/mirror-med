...
async def test_routing_agent():
    ...
    try:
        ...
        if not azure_works:
            print(
                "⚠️  Azure AI connection issues detected. Continuing with routing agent test..."
            )
    except ImportError:
        print(
            "⚠️  Diagnostic script not available. Continuing with routing agent test..."
        )
    ...
    try:
        ...
        try:
            ...
            print(
                "✅ All message processing calls succeeded - HTTP transport issue resolved!"
            )
