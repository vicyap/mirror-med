...
GLOBAL_REPO_CONFIG_FILE = os.path.join(
    os.path.expanduser("~"), ".mcptools", "repo_config.ini"
)
...
def save_repo_path_to_config(repo_name, repo_path):
    ...
    try:
        ...
        logging.info(
            f"Saved repository path to global config: {repo_name} -> {repo_path}"
        )
        return True
...
class GitProgressHandler(git.remote.RemoteProgress):
    ...
    def update(self, op_code, cur_count, max_count=None, message=""):
        ...
        # Log progress
        logging.info(
            f"Clone progress for {self.repo_name}: {progress['current_operation']} - {progress['percent']}%"
        )
...
def open_vscode_mcp(context) -> str:
    ...
    try:
        ...
        if not os.path.exists(path):
            ...
            return json.dumps(
                {"status": "error", "message": f"Error: Path does not exist: {path}"},
                indent=2,
            )
        ...
        try:
            ...
            return json.dumps(
                {"status": "error", "message": f"Error opening VS Code: {str(ex)}"},
                indent=2,
            )
        ...
        return json.dumps({"status": "error", "message": error_message}, indent=2)
    ...
    try:
        ...
        if existing_path and os.path.exists(existing_path) and not custom_path:
            ...
            try:
                ...
                logging.warning(
                    f"Error reading existing repository: {str(ex)}. Will clone again."
                )
        ...
        GLOBAL_REPO_PROGRESS[repo_name] = {"status": "initializing"}
            ...
            for attempt in range(1, retry_attempts + 1):
                try:
                    ...
                    try:
                        ...
                        logging.error(
                            f"Git version check failed: {str(git_version_err)}"
                        )
                        raise Exception(
                            f"Git command not available. Please ensure Git is installed: {str(git_version_err)}"
                        )
                    ...
                    return {"success": True, "repo": repo, "message": success_message}

                except git.GitCommandError as git_err:
                    # Log error information
                    error_info = {
                        "error_type": "GitCommandError",
                        "command": " ".join(git_err.command)
                        if isinstance(git_err.command, list)
                        else git_err.command,
                    ...
                    if os.path.exists(repo_dir):
                        try:
                            shutil.rmtree(repo_dir)
                            logging.info(
                                f"Cleaned up partial repository clone at {repo_dir}"
                            )
                        except Exception as cleanup_err:
                            logging.warning(
                                f"Failed to clean up partial repository: {str(cleanup_err)}"
                            )
